...
/**
 * Service class to handle song fetching and playback
 * Manages song cache, audio playback, and provides utility methods
 * for playing songs normally and in reverse
 * Also handles multi-song playback for mixed modes
 * Uses HTMLAudioElement and Web Audio API for playback
 * Provides methods to fetch songs from backend API
...
export default class SongService {
  ...
  async fetchRandom(genre: Genre = "kpop", count = 50): Promise<Song[]> {
    ...
    console.log(
      `Successfully cached ${this.cachedSongs.length} songs for genre: ${genre}`
    );
  ...
  playSongByDetails(song: Song) {
    ...
    const idx = this.cachedSongs.findIndex(
      (s) =>
        s.title === song.title &&
        s.artist === song.artist &&
        s.previewUrl === song.previewUrl
    );
